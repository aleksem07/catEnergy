...
(function (factory) {
  ...
  function testPadding(parsed, entry) {
    ...
    for (index = 0; index < parsed.spectrum.xNumSteps.length - 1; index++) {
      ...
      if (parsed.padding[0][index] < 0 || parsed.padding[1][index] < 0) {
        throw new Error(
          "noUiSlider (" + VERSION + "): 'padding' option must be a positive number(s)."
        );
      }
    }
    ...
    if (totalPadding / (lastValue - firstValue) > 1) {
      throw new Error(
        "noUiSlider (" + VERSION + "): 'padding' option must not exceed 100% of the range."
      );
    }
  }
  ...
  function testBehaviour(parsed, entry) {
    ...
    if (typeof entry !== "string") {
      throw new Error(
        "noUiSlider (" + VERSION + "): 'behaviour' must be a string containing options."
      );
    }
    ...
    if (fixed) {
      if (parsed.handles !== 2) {
        throw new Error(
          "noUiSlider (" + VERSION + "): 'fixed' behaviour must be used with 2 handles"
        );
      }
    ...
    if (entry === true) {
      ...
      parsed.tooltips.forEach(function (formatter) {
        if (
          ...
          throw new Error(
            "noUiSlider (" + VERSION + "): 'tooltips' must be passed a formatter or 'false'."
          );
        }
      });
    }
  ...
  function scope(target, options, originalOptions) {
    ...
    function getGroup(mode, values, stepped) {
      ...
      if (mode === "count") {
        if (values < 2) {
          throw new Error(
            "noUiSlider (" + VERSION + "): 'values' (>= 2) required for mode 'count'."
          );
        }
    ...
    function generateSpread(density, mode, group) {
      ...
      group.forEach(function (current, index) {
        ...
        for (i = low; i <= high; i = safeIncrement(i, step)) {
          ...
          type =
            group.indexOf(i) > -1 ? PIPS_LARGE_VALUE : isSteps ? PIPS_SMALL_VALUE : PIPS_NO_VALUE;
    ...
    function valueSetHandle(handleNumber, value, fireSetEvent, exactInput) {
      ...
      if (!(handleNumber >= 0 && handleNumber < scope_HandleNumbers.length)) {
        throw new Error(
          "noUiSlider (" + VERSION + "): invalid handle number, got: " + handleNumber
        );
      }
  ...
  function initialize(target, originalOptions) {
    if (!target || !target.nodeName) {
      throw new Error(
        "noUiSlider (" + VERSION + "): create requires a single element, got: " + target
      );
    }
